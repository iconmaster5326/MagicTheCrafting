...
public class ScreenManaTap extends ContainerScreen<ContainerManaTap> {
    ...
    private static ResourceLocation BACKGROUND = new ResourceLocation(
            ...
            PROGRESS_BAR = new ResourceLocation(MineTheCrafting.MOD_ID + ":textures/screen/manatap_progress_bar.png"),
    ...
    protected void drawGuiContainerBackgroundLayer(MatrixStack ms, float ticks, int mouseX, int mouseY) {
        ...
        blit(ms, originX + PROGRESS_X, originY + PROGRESS_Y + (PROGRESS_H - barHeight), 0, PROGRESS_H - barHeight,
                PROGRESS_W, barHeight, PROGRESS_W, PROGRESS_H);

        List<Mana> manas = container.manaGeneratable;
        String part1 = ": Add", part2 = "to your mana pool.";
        int statusIconWidth = STATUS_H + 4;
        int statusWidth = fontRenderer.getStringWidth(part1) + fontRenderer.getStringWidth(part2) + statusIconWidth
                + statusIconWidth * manas.size();
        ...
        for (Mana mana : manas) {
            ...
            blit(ms, statusOriginX + statusIconWidth + fontRenderer.getStringWidth(part1) + statusIconWidth * i + 2, originY + STATUS_Y - 1, 0, 0,
                    STATUS_H, STATUS_H, STATUS_H, STATUS_H);
            i++;
        }

        fontRenderer.drawString(ms, part2,
                statusOriginX + statusIconWidth + fontRenderer.getStringWidth(part1) + statusIconWidth * manas.size(),
